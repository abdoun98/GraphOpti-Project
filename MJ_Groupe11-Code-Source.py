...
######################################################################
#                 ALGORITHME COLONIE DES FOURMIS                     #
######################################################################
#
...
######################################################################
#
# Pour executer 'CDF' (l'algorithme de colonie de fourmis hybridé avec une recherche aléatoire),
...
def fourmiApartirDAleatoire(nbParcour,i, nbFourmis, alpha, beta, Q, omega, coutSelonNbSommet, mcout,mlisteRing, mlisteHorsRing, mlisteLienHorsRing):
...
def CDF(nbParcour, nbFourmis, alpha, beta, Q, omega):
    mcout = math.inf
    ...
    print(time.time()-a)
    while(time.time()-a<1*60):
        n = random.choices(listeNbSommet, probaNbSommet, k=1)[0]
        mcout, mlisteRing, mlisteHorsRing, mlisteLienHorsRing = fourmiApartirDAleatoire(nbParcour, n, nbFourmis, alpha, beta, Q,
                                                                                        omega, coutSelonNbSommet, mcout,
                                                                                        mlisteRing, mlisteHorsRing, mlisteLienHorsRing)
...
def fourmisSurPopulation(Population,Cr,Ca):
    for indv in Population:
        ...
        indv.Cost=cout
        indv.RING=conversionRetour(mlisteRing)[0]
    return Population
...
if __name__ == '__main__':
    ...
    listeRing, listeHorsRing, listeLienHorsRing, cout = CDF(50, 50, 1, 1, 1, 0.95)
    print(cout)
